...
option_list = list(
	...
	make_option(c("-z", "--insert_size"), type="integer", default=199, 
		help="desired length of probes/cis-regulatory sequences containing a SNP ; if indels are retained probe length will differ from selected value. Value - postive integer [default= %default]", metavar="integer"),
	make_option(c("-c", "--n_controls"), type="integer", default=100, 
		help="The number of control sequences to design. One control = 4 oligos; Fwd/Rev : Allele1/Allele 2. Value positive integer [default= %default]", metavar="integer") ,
	make_option(c("-e", "--ChIP_enrichment"), type="numeric", default=4, 
 		help="If providing a control region file only design control probes/CRSs in regions with an enrichment > than this value. [default= %default]", metavar="numeric"),	
	...
	make_option(c("-P", "--P_value_cuttoff"), type="numeric", default=5e-8, 
		help="hard P-value cut-off. Variants with a P-value above this value are removed. [default= %default]", metavar="numeric"),
	...
	make_option(c("-S", "--SNVs_only"), type="logical", default=T, 
		help="retain only single nucleotide variants [default= %default]", metavar="logical"),
	...
	make_option(c("--update_alleles"), type="logical", default=F, 
		help="If T when user provided variant alleles dont match the references dbSNPs extract alleles from the reference  [default= %default]", metavar="logical"),	
	...
	make_option(c("--split_by_r2"), type="logical", default=T, 
		help="if T variants in a region surrounding a sentinel SNP will be split into separate loci based on linkage. The SNP with the lowest p-value in a new loci will be the new lead and used for p-value filtering. [default= %default]", metavar="logical"),
	make_option(c("-r", "--r2_cuttoff"), type="numeric", default=0.3, 
		help="when splitting loci variants with an r2 , to the lead SNP, below this value are split into separate loci. Value - 0-1 [default= %default]", metavar="numeric"),
	...
	make_option(c("--filter_homopolymers"), type="logical", default=T, 
		help="probes with homopolymers (specified by --max_hp_len) will be filtered. If --mutate_homopolyers is T successfully mutated homopolymers are retained [default= %default]", metavar="logical") ,
	make_option(c("-l", "--min_dist_mut2snp"), type="integer", default=5, 
		help="bases within a homoploymer < this number of base pairs from the reference or alternative allele are not mutated.. Value - positive integer [default= %default]", metavar="integer"),
...
allele_A_is_B <-(loci_SNPs2$allele_A == loci_SNPs2$allele_B) | ((! sapply(1:length(loci_SNPs2$allele_A), function(i) all(unlist(strsplit(loci_SNPs2$allele_A[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$allele_B), function(i) all(unlist(strsplit(loci_SNPs2$allele_B[i],"")) %in% c("A","T","C","G","a","t","c","g")))))
...
if (any (is.na(matched_rsid))) {
	...
	if (update_alleles) {
		...
		#snps_same<-sapply(names(multi_allele), function(i) {
		#	SNProw<-subjectHits(dbsnps.ol)[queryHits(dbsnps.ol) == i]
		#	length(unique(dbSNPs.gr$REF[SNProw])) == 1 & length(unique(unlist(dbSNPs.gr$ALT[SNProw]))) == 1
		#})
	...
	#matched_alleles.df<-data.frame(index=ind, allele_match=same_alleles,missing_row=queryHits(dbsnps.ol),db_snp_row=subjectHits(dbsnps.ol))
	...
	#matched_alleles_TF<-sapply(unique(ind), function(i) { 
	#	any(matched_alleles.df$allele_match[matched_alleles.df$index == i])
	#})
	#mismatch_alleles_ind<-ind[! same_alleles]
...
if ( length(ref_is_alt) > 0 ) {
	...
	ref_alt_illegal<-which((! sapply(1:length(loci_SNPs2$allele_A), function(i) all(unlist(strsplit(loci_SNPs2$allele_A[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$allele_B), function(i) all(unlist(strsplit(loci_SNPs2$allele_B[i],"")) %in% c("A","T","C","G","a","t","c","g")))))
...
still_nok<-loci_SNPs2$REF == loci_SNPs2$ALT  | (( ! sapply(1:length(loci_SNPs2$REF), function(i) all(unlist(strsplit(loci_SNPs2$REF[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$ALT), function(i) all(unlist(strsplit(loci_SNPs2$ALT[i],"")) %in% c("A","T","C","G","a","t","c","g")))))
...
#else if (! keep_homopolymers) {  # just remove any homopolymers 

#	if ( mutate_homopolyers ) { #
#	} else {
#		homopolymer_filter_ind<-unique(c(has_homo_ref_ind,has_homo_alt_ind))
