...
option_list = list(
	...
	make_option(c("-u", "--unfiltered_file_path"), type="character", default=NULL, 
		help="path to files containing unfiltered variants, see instructions for more information relating to format, to add proxies this must be provided, optional input [default= %default]"),
	...
	make_option(c("-s", "--suffix"), type="character", default="", 
		help="[character] Suffix added to ouput file names [default= %default]", metavar="ouput file suffix"),
	make_option(c("-T", "--API_token"), type="character", default=NULL, 
		help="[character] Token to access LDlink database see 'https://ldlink.nci.nih.gov/?tab=apiaccess' to obtain", metavar="API_token"),
	make_option(c("-F", "--Forward_adapter"), type="character", default="AGGACCGGATCAACT", 
		help="[character] Sequence added to the 5\\' end of each oligo [default= %default]", metavar="Forward adapter sequence") ,
	make_option(c("-R", "--Reverse_adapter"), type="character", default="CATTGCGTGAACCGA", 
		help="[character] Sequence added to the 3\\' end of each oligo [default= %default]", metavar="Reverse adapter sequence") ,
	make_option(c("-w", "--loci_window"), type="integer", default=250000, 
		help="[integer] All variants less than this number of basepairs from a lead SNP will be considered for designing probes. Value - postive integer [default= %default]", metavar="loci genome window size"),
	make_option(c("-z", "--insert_size"), type="integer", default=199, 
		help="[integer] Length of probes/cis-regulatory sequences containing a SNP, not including adapters (base pairs). If probes contain indels length will differ from selected value. Value - postive integer [default= %default]", metavar="insert size"),
	make_option(c("-c", "--n_controls"), type="integer", default=100, 
		help="[integer] The number of control sequences to design. One control = 4 oligos; Fwd/Rev : Allele1/Allele 2. Value positive integer [default= %default]", metavar="number of control sequences") ,
	make_option(c("-e", "--ChIP_enrichment"), type="numeric", default=4, 
 		help="[numeric] If providing a control region file only design control probes/CRSs in regions with an enrichment > than this value. [default= %default]", metavar="ChIP enrichment score"),	
	make_option(c("--prop_control"), type="numeric", default=0.5, 
 		help="[numeric] If providing a control region file what proportion of controls sequences should be from this. Value - 0-1 [default= %default]", metavar="proportion of controls from negative control region"),		
	make_option(c("-m", "--minor_allele_frequency"), type="numeric", default=0.01, 
		help="[numeric] variants with a MAF/RAF lower than this value are removed. Value 0-1 [default= %default]", metavar="minimum minor allele frequency"),
	make_option(c("-P", "--P_value_cuttoff"), type="numeric", default=5e-8, 
		help="[numeric] Hard P-value cut-off. Variants with a P-value above this value are removed. [default= %default]", metavar="min P value"),
	make_option(c("-f", "--P_cuttoff_factor"), type="numeric", default=0.7, 
		help="[numeric] P_value thresholds for each risk loci are caluated as -log(lead_Pval,10)*P_cuttoff_factor. Value 0-1  [default= %default]", metavar="locus P value cuttoff factor") ,
	make_option(c("-S", "--SNVs_only"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] Retain only single nucleotide variants [default= %default]", metavar="filter indels"),
	make_option(c("-L", "--max_indel_length"), type="integer", default=10, 
		help="[integer] Indels with an alt allele > this value are removed; irrelevent if -S is T. Value - postive integer [default= %default]", metavar="max indel length"),
	make_option(c("-U", "--keep_unmapped_variants"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] Retain user provided variants not present in reference SNP file [default= %default]", metavar="keep unmapped variants"),
	make_option(c("--update_alleles"), type="logical", default=F, 
		help="[logical T:F, TRUE:FALSE] If T when user provided variant alleles dont match the references dbSNPs extract alleles from the reference  [default= %default]", metavar="update alleles"),	
	make_option(c("--output_figures"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] Generate figures showing filtered and retained variants [default= %default]", metavar="output figures"),
	make_option(c("--split_by_r2"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] If TRUE variants in a region surrounding a sentinel SNP will be split into separate loci based on linkage. The SNP with the lowest p-value in a new loci will be the new lead and used for p-value filtering. [default= %default]", metavar="split by r2"),
	make_option(c("-r", "--r2_cuttoff"), type="numeric", default=0.3, 
		help="[numeric] When splitting loci variants with an r2 , to the lead SNP, below this value are split into separate loci. Value - 0-1 [default= %default]", metavar="loci split r2 cuttoff"),
	make_option(c("--max_hp_len"), type="integer", default=8, 
		help="[integer] Homopolmer (mono nucleotide) runs > than is value (base pairs) are considered homopolymers. Value - positive integer [default= %default]", metavar="maximum homopolmer length"),
	make_option(c("-M", "--mutate_homopolyers"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] Probes with homopolymers (specified by --max_hp_len) will be mutated to remove homopolymers. If F any homopolymers will be filtered [default= %default]", metavar="mutate homopolyers") ,
	make_option(c("--filter_homopolymers"), type="logical", default=T, 
		help="[logical T:F, TRUE:FALSE] Probes with homopolymers (specified by --max_hp_len) will be filtered. If --mutate_homopolyers is T successfully mutated homopolymers are retained [default= %default]", metavar="filter homopolymers") ,
	make_option(c("-l", "--min_dist_mut2snp"), type="integer", default=5, 
		help="[integer] Bases within a homoploymer < this number of base pairs from the reference or alternative allele are not mutated.. Value - positive integer [default= %default]", metavar="minimum distance introduced mutation to SNP"),
	make_option(c("-p", "--populations"), type="character", default="EUR", 
		help="[character] Populations used for r2 extraction one or more of 'EUR,SAS,EAS,AMR,AFR' If multiple populations separate by a comma [default= %default]", metavar="populations"),
	make_option(c("-x", "--add_proxies"), type="logical", default=F, 
		help="[logical T:F, TRUE:FALSE] Import variants linked to a lead SNP but absent from user provided variants [default= %default]", metavar="add proxies"),
	make_option(c("--proxy_maf"), type="numeric", default=0.01, 
		help="[numeric] Filter proxies with a maf less than this value. Value 0-1  [default= %default]", metavar="proxy maf cuttoff"),
	make_option(c("--proxy_r2"), type="numeric", default=0.7, 
		help="[numeric] Filter proxies with a r2, to loci lead, less than this value. Value 0-1 [default= %default]", metavar="proxy_r2"),
	make_option(c("--filter_phet"), type="logical", default=F, 
		help="[logical T:F, TRUE:FALSE] Where lead SNPs has phet > value specified in P_het_cuttoff option (ie not heterogenous) filter variants that have phet < P_het_cuttoff. If F no filtering is performed [default= %default]", metavar="filter p heterogeneity"),
	make_option(c("--P_het_cuttoff"), type="numeric", default=0.01, 
		help="[numeric] If --filter_phet = T. The phet cutoff for filtering set by this value. Value 0-1 [default= %default]", metavar="Phet cuttoff"),
	make_option(c("-C", "--cycle_limit"), type="integer", default=5, 
		help="[integer] Risk loci are split recursively into loci containing only linked variants, this value sets the upper limit on the number of cycles used to split loci. Value - positve integer [default= %default]", metavar="cycle limit"),
	make_option(c("--strict"), type="logical", default=F, 
		help="[logical T:F, TRUE:FALSE] If TRUE script will exit where it encounters issues that while may indicate some errors, but do not prevent execution [default= %default]", metavar="strict")
)
...
update_alleles=opt$update_alleles
...
filter_homopolymers=opt$filter_homopolymers
...
nec_cols=c("chr", "pos", "rsid", "P_value","allele_A","allele_B")
if ( ! all(nec_cols %in% colnames(main_SNPs))) stop ("all_SNP_file must contain the columns /chr/pos/rsid/P_value")
...
allele_A_is_B <-(loci_SNPs2$allele_A == loci_SNPs2$allele_B) | ((! sapply(1:length(loci_SNPs2$allele_A), function(i) all(unlist(str_split(loci_SNPs2$allele_A[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$allele_B), function(i) all(unlist(str_split(loci_SNPs2$allele_B[i],"")) %in% c("A","T","C","G","a","t","c","g")))))
if ( strict & any(allele_A_is_B)) {
	cat (sep="\n", loci_SNPs2$rsid[allele_A_is_B])
	stop("Allele A and Allele_B are either the same or have non DNA characters for the above SNPs")
}
...
#match user supplied snps to ref db snps on rsid
...
# for those that dont match look up based on position alleles
if (any (is.na(matched_rsid))) {
	...
	if ( any ( number_mappings > 1)) {
		message ( "the following SNPs have more than one variant at the same position" )
		var<-as.numeric(names(number_mappings )[number_mappings > 1])
	...
	no_pos_match_ind<-as.numeric(unmatched_ids)[ ! as.numeric(unmatched_ids) %in% ind] # index of SNPs with no rsid and no position match

	if ( any(no_pos_match_ind %in% as.numeric(names(matched_rsid)[! is.na(matched_rsid)]))) stop("interal error no_pos_match_ind and as.numeric(names(matched_rsid)[! is.na(matched_rsid)]) have common indicies. Please report")

	missing_id_allele_A<-loci_SNPs2$allele_A[ind]
	missing_id_allele_B<-loci_SNPs2$allele_B[ind]
	
	id_lookup.df<- data.frame(loci_SNPs_ind=ind,queryHits=queryHits(dbsnps.ol), subjectHits=subjectHits(dbsnps.ol), stringsAsFactors=F)
	...
	#return a vector of those SNP index where the alleles match between the user input and the dbSNP file
	same_alleles<-((missing_id_allele_A == ref & sapply(1:length(ind), function(i) any(missing_id_allele_B[i] == alt_list[[i]]))) |
	(missing_id_allele_B == ref & sapply(1:length(ind), function(i) any(missing_id_allele_A[i] == alt_list[[i]]))))

	allele_A_is_B_ind<- which(allele_A_is_B)
	#get ind allele A == B where there was a position and allele match
	allele_A_is_B_ind_w_pos_match<-allele_A_is_B_ind[allele_A_is_B_ind %in% ind]
	#get ind allele A == B where there was no position and no allele match
	allele_A_is_B_ind_wo_pos_match<-allele_A_is_B_ind[ (! allele_A_is_B_ind %in% ind) & (! allele_A_is_B_ind %in% names(matched_rsid)[! is.na(matched_rsid)])]
	
	new_name_ind<-subjectHits(dbsnps.ol)[same_alleles]
	new_names<-names(dbSNPs.gr)[new_name_ind]
	
	not_same_alleles_ind<-c(unique(ind[ ! same_alleles ]),allele_A_is_B_ind_w_pos_match)

	# if there is a position match and update_alleles = T then take allele info from dbSNPs  
	if (update_alleles) {
		multi_allele<-number_mappings[number_mappings > 1]
		
		snps_alleles<-lapply(as.numeric(names(multi_allele)), function(i) {
			SNProw<-subjectHits(dbsnps.ol)[queryHits(dbsnps.ol) == i]
			refy=unique( as.character(unlist(dbSNPs.gr$REF[SNProw])))
			alty=unique( as.character(unlist(dbSNPs.gr$ALT[SNProw])))
			return(list(refy,alt))
		})
		names(snps_alleles)<-names(multi_allele)

		snps_same<-sapply(names(snps_alleles), function(i) {
		length(snps_alleles[[i]][[1]]) == 1 & length(snps_alleles[[i]][[2]]) == 1
		})

		dont_update<-as.numeric(names(multi_allele)[ ! snps_same ])
		dont_update_loci_SNPs2_ind<-unique(id_lookup.df$loci_SNPs_ind[id_lookup.df$queryHits == dont_update])

		not_same_alleles_update_ind<-not_same_alleles_ind[ ! not_same_alleles_ind %in% dont_update_loci_SNPs2_ind]
		#not_same_alleles_ind<-match(not_same_alleles_ind,unmatched_ids)
		
		loci_SNPs2$REF[not_same_alleles_update_ind]<-sapply(not_same_alleles_update_ind, function(i) {
			SNProw<-id_lookup.df$subjectHits[id_lookup.df$loci_SNPs_ind == i]
			refy=unique( as.character(unlist(dbSNPs.gr$REF[SNProw])))
			if(length(refy) > 1) stop("ref_allele is not unique, internal error please report")
			return(refy)
		})
		
		loci_SNPs2$ALT[not_same_alleles_update_ind]<-sapply(not_same_alleles_update_ind, function(i) {
			SNProw<-id_lookup.df$subjectHits[id_lookup.df$loci_SNPs_ind == i]
			refy=unique( as.character(unlist(dbSNPs.gr$ALT[SNProw])))
			if(length(refy) > 1) stop("ref_allele is not unique, internal error please report")
			return(refy)
		})

	#the index of those SNPs where we couldnt be update the alleles 
	not_updated<-not_same_alleles_ind[ not_same_alleles_ind %in% dont_update_loci_SNPs2_ind]
	unmapped_remove<-allele_A_is_B_ind_w_pos_match[! allele_A_is_B_ind_w_pos_match %in% not_same_alleles_update_ind ]

	} else if ( ! update_alleles ) {
		unmapped_remove<- allele_A_is_B_ind_w_pos_match
		not_updated<-not_same_alleles_ind
	}

	must_filt<-unique(c(allele_A_is_B_ind_wo_pos_match,unmapped_remove))
	new_REF=ref[same_alleles]
	new_ind<-ind[same_alleles]

	matched_alleles<-unique(new_ind)
	unmatched_alleles<-unique(ind)[ ! unique(ind) %in% matched_alleles ]
	update_match<-data.frame(new_name_ind=new_name_ind,SNP_ind=new_ind)

	#update the matched_rsid vector with the new matches based on position and allele match
	matched_rsid[update_match$SNP_ind]<- update_match$new_name_ind

	new_ALT<-sapply( 1:length(new_ind), function(i) { 
		ind=new_ind[i] 
		ref_alt<-c(loci_SNPs2$allele_A[ind],loci_SNPs2$allele_B[ind])
	...
	# ind of lociSNPs that dont have either a rsid match or a postition match
	not_remapped <- all_missing [ ! all_missing %in% queryHits(dbsnps.ol)]
	#not_remapped <- all_missing [ ! all_missing %in% queryHits(dbsnps.ol)[same_alleles]]
	not_remapped_ind<-as.numeric(unmatched_ids[not_remapped])
	# now remove no match snps if desired
	if ( keep_unmapped_variants == T ) 
	{
		message( "the following SNPs rsids are not in the reference file but will be retained") 
		cat("\n")
		kept<-not_remapped_ind[! not_remapped_ind %in% must_filt]
		#cat(sep="\n",loci_SNPs2$rsid[not_remapped_ind])
		cat(sep="\n",loci_SNPs2$rsid[kept])
		#cat(sep="\n",loci_SNPs2$rsid[unmatched_alleles])
		remove_ind=c()
		cat ( "------------------------------------------")
		cat("\n")
		...
		cat("\n")
		filt_unmapped<-c(not_remapped_ind,not_updated)
		cat(sep="\n",loci_SNPs2$rsid[filt_unmapped])
		#remove_ind<-c(not_remapped_ind,unmatched_alleles)
		remove_ind<-c(filt_unmapped)
		cat ( "------------------------------------------")
		cat("\n")
	}
	
	if ( length(must_filt) > 1 ) {
		message( "the following SNPs have the same allele_A and allele_B or non DNA characters and cant be revised based on the reference file and will be filtered") 
		cat("\n")
		cat(sep="\n",loci_SNPs2$rsid[must_filt])
		#cat(sep="\n",loci_SNPs2$rsid[unmatched_alleles])
		#remove_ind<-c(not_remapped_ind,unmatched_alleles)
		remove_ind<-unique(c(remove_ind,must_filt))
		cat ( "------------------------------------------")
		cat("\n")
	}
}

# print out those rsids that were updated
...
allele_A<-loci_SNPs2$allele_A[as.numeric(names(matched_rsid))] 
allele_B<-loci_SNPs2$allele_B[as.numeric(names(matched_rsid))] 

# get the ref and alt from the refdb file
ref<-as.character(dbSNPs.gr$REF[matched_rsid])
alt_list<-as.list(dbSNPs.gr$ALT[matched_rsid])
alt_list<-lapply(alt_list, function(alleles) as.character(alleles))

# check if the alleles match between the 
...
not_same_alleles_ind<-as.numeric(names(matched_rsid))[ ! same_alleles ]
...
# double check all the alleles match
all_mismatches<- ! sapply(ref_alt_ind , function(ind) {
	alleleA=loci_SNPs2$allele_A[ind]
	alleleB=loci_SNPs2$allele_B[ind]
...
missmatch_ind<-ref_alt_ind[all_mismatches]
not_same_alleles_ind<-unique(c(not_same_alleles_ind,missmatch_ind))
snps_2_ref_alt_ind<-match(not_same_alleles_ind,names(matched_rsid))
...
if ( length(not_same_alleles_ind) > 1 & update_alleles) {
 
	gwas_rsid=loci_SNPs2$rsid[not_same_alleles_ind]
	dbsnp_rsid<-names(dbSNPs.gr)[matched_rsid[snps_2_ref_alt_ind]]

	if ( ! all( gwas_rsid == dbsnp_rsid )) stop( "Internal error ; the object matching gwas and db snps is not opperating correctly, please report") 
 
	is_biallelic<- sapply(1:length(alt_list[snps_2_ref_alt_ind]), function(i) length(alt_list[snps_2_ref_alt_ind][[i]]) == 1)
	loci_SNPs2$REF[not_same_alleles_ind[is_biallelic]]<-ref[snps_2_ref_alt_ind[is_biallelic]]
	loci_SNPs2$ALT[not_same_alleles_ind[is_biallelic]]<-sapply(alt_list[snps_2_ref_alt_ind[is_biallelic]], function(slot) slot[[1]])
	updated_alleles<-not_same_alleles_ind[is_biallelic]
	#triallelic_filt_ind<-not_same_alleles_ind[!is_biallelic]
	not_same_alleles_ind<-not_same_alleles_ind[!is_biallelic]

	message( "The provided REF and ALT alleles for the following SNP(s) have been updated") 
	cat("\n")
	cat(sep="\n",loci_SNPs2$rsid[updated_alleles])
	cat ( "------------------------------------------")
	cat("\n")
} 

#in case some allele_A and allele_B are the same remove these
#ref_is_alt<-which(loci_SNPs2$allele_A == loci_SNPs2$allele_B  | (( ! sapply(1:length(loci_SNPs2$allele_A), function(i) all(unlist(strsplit(loci_SNPs2$allele_A[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | ( ! sapply(1:length(loci_SNPs2$allele_B), function(i) all(unlist(strsplit(loci_SNPs2$allele_B[i],"")) %in% c("A","T","C","G","a","t","c","g")))))) 

ref_is_alt<-which(allele_A_is_B)
ref_is_alt<-ref_is_alt[ref_is_alt %in% names(matched_rsid)]

if ( length(ref_is_alt) > 0 ) {

	gwas_rsid=loci_SNPs2$rsid[ref_is_alt]
	dbsnp_rsid<-names(dbSNPs.gr)[matched_rsid[as.character(ref_is_alt)]]

	if ( ! all( gwas_rsid == dbsnp_rsid )) stop( "the object matching gwas and db snps is not opperating correctly") 

	ref_sub<-as.character(dbSNPs.gr$REF[matched_rsid[as.character(ref_is_alt)]])
	alt_sub<-as.list(dbSNPs.gr$ALT[matched_rsid[as.character(ref_is_alt)]])
	alt_sub<-lapply(alt_sub, function(alleles) as.character(alleles))

	is_biallelic<- sapply(alt_sub, function(alt) length(alt) == 1)

	loci_SNPs2$REF[ref_is_alt[is_biallelic]]<-ref_sub[is_biallelic]
	loci_SNPs2$ALT[ref_is_alt[is_biallelic]]<-unlist(alt_sub[is_biallelic])

	#filter snps with illegal characters in REF ALT
	ref_alt_illegal<-which((! sapply(1:length(loci_SNPs2$allele_A), function(i) all(unlist(str_split(loci_SNPs2$allele_A[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$allele_B), function(i) all(unlist(str_split(loci_SNPs2$allele_B[i],"")) %in% c("A","T","C","G","a","t","c","g")))))

	ref_alt_remove<-ref_alt_illegal[ ! ref_alt_illegal %in% names(matched_rsid)]
	#ref_alt_remove<-ref_alt_illegal[ ! rownames(loci_SNPs2)[ref_alt_illegal] %in% names(matched_rsid)] # if rows have been filtered

	if ( length(ref_is_alt[is_biallelic]) > 0) {
		message( "User provided allele_A and allele_B were the same or contained non DNA characters for the follwoing SNP(s) and have been updated based on the refernece SNPdb") 
		cat("\n")
		cat(sep="\n",loci_SNPs2$rsid[ref_is_alt[is_biallelic]])
		cat ( "------------------------------------------")
		cat("\n")
	}

	if (length(ref_is_alt[! is_biallelic]) > 0 | length(ref_alt_remove) > 0) {
		message( "User provided allele_A and allele_B were the same or contained non DNA characters for the follwoing SNP(s) and CANT be updated based on the refernece SNPdb") 
		cat("\n")
		cat(sep="\n",loci_SNPs2$rsid[sort(unique(c(ref_is_alt[! is_biallelic],ref_alt_remove)))])
		cat ( "------------------------------------------")
		cat("\n")
		remove_ind<-sort(unique(c(remove_ind,ref_is_alt[! is_biallelic],ref_alt_remove)))
	}
}	

# remove those that were fixed from the remove list
not_same_alleles_ind<-not_same_alleles_ind[ ! not_same_alleles_ind %in% ref_is_alt[is_biallelic] ]
not_same_alleles_ind_kept<-not_same_alleles_ind[ ! not_same_alleles_ind %in% remove_ind]

if ( keep_unmapped_variants & length(not_same_alleles_ind_kept) > 0 ) {
	message( "The provided REF and ALT alleles for the following SNP(s) dont match the dbSNP file, but will be retained") 
	cat("\n")
	cat(sep="\n",loci_SNPs2$rsid[not_same_alleles_ind_kept])
	cat ( "------------------------------------------")
	cat("\n")
} else if ( ! keep_unmapped_variants ) {
	remove_ind<- unique(c(remove_ind,not_same_alleles_ind))
	message("The provided REF and ALT alleles for the following SNP(s) dont match the dbSNP file, and will be filtered") 
	cat("\n")
	cat(sep="\n",loci_SNPs2$rsid[not_same_alleles_ind])
	cat ( "------------------------------------------")
	cat("\n")
}
...
if ( length( no_ref_alt_ind) > 0) {
	...
	ref_chr<-unique(substr(names(ref_genome),1,3))
	if ( "chr" %in% ref_chr) { 
		chrom<-add_chr(chrom)
	} else if (unique(substr(chrom,1,3)) == "chr" & length(unique(substr(chrom,1,3))) == 1) {
		names(ref_genome)<-gsub( "^","chr",names(ref_genome))
	}

	snps<-subseq(ref_genome[chrom],start=pos,end=pos)
	snps_ref<-sapply(as.character(snps), function(snp) substr(snp,1,1))

	allele_A_is_ref<-snps_ref == loci_SNPs2$allele_A[no_ref_alt_ind]
	allele_B_is_ref<-snps_ref == loci_SNPs2$allele_B[no_ref_alt_ind]

	ok_ref<-which(allele_A_is_ref | allele_B_is_ref)
	...
	allele_A_is_ref_ind<-no_ref_alt_ind[allele_A_is_ref]
	allele_B_is_ref_ind<-no_ref_alt_ind[allele_B_is_ref]

	loci_SNPs2$REF[allele_A_is_ref_ind]<-loci_SNPs2$allele_A[allele_A_is_ref_ind]
	loci_SNPs2$ALT[allele_A_is_ref_ind]<-loci_SNPs2$allele_B[allele_A_is_ref_ind]

	loci_SNPs2$REF[allele_B_is_ref_ind]<-loci_SNPs2$allele_B[allele_B_is_ref_ind]
	loci_SNPs2$ALT[allele_B_is_ref_ind]<-loci_SNPs2$allele_A[allele_B_is_ref_ind]
}
...
if ( length(no_ref_alt_ind) >  0) {
	...
	if (strict) stop ( "Exiting,to randomly assign ref and alt change strict = F")
	
	cat (" this may lead to unusual behavior\n")
	loci_SNPs2$REF[no_ref_alt_ind]<- loci_SNPs2$allele_A[no_ref_alt_ind]
	loci_SNPs2$ALT[no_ref_alt_ind]<- loci_SNPs2$allele_B[no_ref_alt_ind]
	cat ( "------------------------------------------")
	cat("\n")
}
...
still_nok<-loci_SNPs2$REF == loci_SNPs2$ALT  | (( ! sapply(1:length(loci_SNPs2$REF), function(i) all(unlist(str_split(loci_SNPs2$REF[i],"")) %in% c("A","T","C","G","a","t","c","g")))) | (! sapply(1:length(loci_SNPs2$ALT), function(i) all(unlist(str_split(loci_SNPs2$ALT[i],"")) %in% c("A","T","C","G","a","t","c","g")))))

if (any(still_nok)) { 
#save.image(".RData")
stop ("Internal error; some REF and ALT alleles are still either the same or contain non DNA characters, please report")
}
...
if (split_by_r2 == T) {
	...
	next_pval_snps_rounds=list()
	while (length(both_missing) > 0) {
		# for any snp where we were unable to extract linked snps look up the allele_Brest p-val snp in the same locus
		next_pval_snps<- sapply(both_missing, function(snp) { 
			lead_snp=unique(loci_SNPs2$lead[loci_SNPs2$rsid== snp ])
			loc_snps<-filter(loci_SNPs2 , lead == {{ lead_snp }})
			lead_p<-loci_SNPs2$P_value[loci_SNPs2$rsid == lead_snp ]
	...
	loci_SNPs3<-as.data.table(loci_SNPs2)
}
cat("\n")

if ( ! "comments" %in% colnames( loci_SNPs3) ) loci_SNPs3$comments <- NA

prefilt_snps<-loci_SNPs3
...
if( add_proxies == T  & (! is.null(unfiltered_file_path)) & split_by_r2 == T) { 
	...
	chr_proxies_filt<-lapply(unique(proxies_merge$chr), function(chr) {
		...
		chr_proxies_filt<-sapply(chr_leads, function(lead) {
			...
			unfilt_loci_match1<-paste(unfilt_loci_snps$chr,unfilt_loci_snps$pos,unfilt_loci_snps$allele_A,unfilt_loci_snps$allele_B,sep="_")
			unfilt_loci_match2<-paste(unfilt_loci_snps$chr,unfilt_loci_snps$pos,unfilt_loci_snps$allele_B,unfilt_loci_snps$allele_A,sep="_")
	...
	cols_keep_gwas<-cols_keep_gwas[cols_keep_gwas %in% colnames(loci_SNPs3)]
	gwas_snps_merge<-loci_SNPs3[,..cols_keep_gwas]
	if (str_sub(gwas_snps_merge$chr[1],0,3) == "chr")
	gwas_snps_merge$chr<-gsub("chr","",gwas_snps_merge$chr)
...
###############################
# remove homopolymers (> max_hp_len bases) due to higher synthesis error rate.
...
any_hps<-length(has_homo_alt_ind) > 0 & length(has_homo_ref_ind) > 0

if ( mutate_homopolyers == T & any_hps) {
	...
	# identify those where the changes on the ref and alt are not the same
	...
	#for those sequeneces where the changes made to the ref and alt are not the same try mapping the ref changes to the alt and vice versa.
	...
	# of those sequneces with different changes to the ref and alt where we tried to fix by swapping the changes get those that we were able to change sucessfully
	keep=! is.na(col)
	keep_change_ind<-diff_change_ind[keep]

	# of those sequences that we rescued up date the ref alt mod col
	...
	successfully_changed<- as.numeric(names(count_ind)[! as.numeric(names(count_ind)) %in% remove_change_ind ])
	# this doesnt work as some have been modified but not successfully 
	#if ( ! all( successfully_changed %in% which((! is.na(all_variants$REF_mod)) & (! is.na(all_variants$ALT_mod))))) stop ( "interal error, successfully_changed variable doesnt match is.na(all_variants$[REF|ALT]_mod)")
	cat ( "------------------------------------------")
	cat ("\n")
...
if ( ! filter_homopolymers) {
	if ( mutate_homopolyers ) { # use the modified keep the others
		...
		use_orig<-c(use_orig,remove_change_ind)
		#use_orig <- use_orig[ ! use_orig %in% cant_use_homo]

		use_mod<- which(mod_ref & mod_alt)
		use_mod<-use_mod[ ! use_mod %in% remove_change_ind] # filter those oligos that were not sucessfully filtered 

		if ( length(use_mod) ) {
			message( "the following SNPs have have been edited to remove homopolymers") 
			cat(sep="\n",all_variants$rsid[use_mod])
			cat("\n")
		}
		...
		if ( length(cant_use_homo) ) {
			message( "the following SNPs have have been removed as they contain homopolymers") 
			cat(sep="\n",all_variants$rsid[cant_use_homo])
			cat ( "------------------------------------------")
			cat("\n")
		}
	}
} else if (filter_homopolymers) {
	if ( mutate_homopolyers ) { # filter those sequences that couldn't be mutated
	...
	} else if ( ! mutate_homopolyers ) { # filter all homopolymers

		all_variants$use_modified<-NA
		cant_use_homo <- homopolymer_filter_ind
		all_variants$comments<-append_to_column(all_variants$comments,"contains homoployer",cant_use_homo)
		use_orig<-which(is.na(all_variants$comments))
		homopolymer_filter<-all_variants[cant_use_homo,]
		if ( length(cant_use_homo) ) {
...
if ( any(ref_sceI_TF) ) {
	stop ( "some ref oloigos have an SceI site in them, mine didnt so I didnt code for this, please report")
}

alt_sceI_TF<-sapply(SceI_alt_matches, function(row) length(row)) > 0
if ( any(alt_sceI_TF) ) {
	stop ( "some alt oloigos have an SceI site in them, mine didnt so I didnt code for this, please report")
}
...
if(length(has_homo)) {
	...
	for ( snp in unique(final_update$rsid)) { 
		...
		if( ! nrow(changed_seqs) %% 2 == 0 ) stop ( "Edited and odd number of oliogs for ", snp, " must change both ref and alt, internal error please report") 

		if (all(changed_seqs$keep)) {
			if ( ! (sum ( fwd_rev == "fwd") == 2 | sum ( fwd_rev == "rev") == 2 )) stop ("final update error, please report")	
	...
	if ( ! (all(all_variants$rsid[update_all_var_ind] %in% names(library_adapters[update_library_ind])) & all(all_variants$rsid[update_all_var_ind] %in% test_control_SNPs$rsid[update_test_cont_ind]))) stop ( "all_variants vs oligo library mismatch, internal error please report")
} else {

 update_library_ind<-c()
 #all_var_ind
...
all_pre_P_filt<-unique(prefilt_snps$rsid)

#do a little characterisation 
u_p_snp<-unique(p_val_lead_snp)

message("	generating summary")
numbers_remaining<-lapply(lead_snps, function(lead) {
	pre_snps<-filter(prefilt_snps, lead == {{lead}})
	if ( ! lead %in%  all_variants$rsid ) {
		print(lead)
		message( lead, " is not in all_variants$rsid" )
		if ("orig_lead" %in% colnames(all_variants)) {
			post_snps<-all_variants[all_variants$orig_lead == lead,]
		} else {
			post_snps<-all_variants[all_variants$pre_filt_lead == lead,]
		}
...
numbers_remining<-data.frame(lead_snp=lead_snps, p_val_lead = p_val_lead,original_n = orig, post_p_filtering = post_p, post_qc_filtering=post_qc,old_max_p=max_old_p,old_min_p=min_old_p, new_max_p=max_new_p)
...
fwrite(prefilt_snps,file.path(outdir,paste0("MPRA_prefilter_sequences",suffix,".txt")), sep="\t",row.names=F, quote=F)
fwrite(test_control_SNPs,file.path(outdir,paste0("MPRA_library_included_variants_and_cotrols",suffix,".txt")),sep="\t",row.names=F, quote=F)
...
if ( output_figures == T ) { 
	...
	for ( snp in lead_snps)  {
	  old_snps<-filter(prefilt_snps,  lead == snp)
